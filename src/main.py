...
class AlienInvasion:
    ...
    def _collision(self):
        ...
        collisions = pygame.sprite.groupcollide(self.bullets, self.aliens,
                                                True, True)
        # Adding score values on collision
        if collisions:
    ...
    def _update_screen(self):
        ...
        if not self.game_active and self.stats.ships_left > 0:
            ...
            self.stats.draw_game_over()
            self.reset_button.draw_button()

        # Displaying game level and score
        self.stats.draw_stats("Level: " + (str(self.stats.level)),
                              self.settings.stat_color_level, "top_left")
        self.stats.draw_stats("Score: " + str(self.stats.score),
                              self.settings.stat_color_score, "top_center")
        self.stats.draw_stats("Lives: " + str(self.stats.ships_left*"❤"),
                              self.settings.stat_color_lives, "top_right")
